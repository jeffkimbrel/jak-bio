...
phyloOther = function(p, level = "family", min = 10, xAxis = "SAMPLE") {
  ...
  tPalette = ""
  if (level == "phylum") {
    tPalette = phylumPalette

  } else if (level == "family") {
    tPalette = familyPalette

  } else if (level == "genus") {
    tList = as.data.frame(unique(tax_table(phylo)[,"genus"]))
    tList = filter(tList, genus != "_Other")
    tList = as.vector(tList$genus)
    tPalette = getPalette(min)
    names(tPalette) = tList
    tPalette["_Other"] = "black"
  ...
  plot = plot_bar(phylo, fill = level, x = xAxis) +
    scale_fill_manual(values = tPalette) +
    ggtitle(paste0(levelUpper," Glommed"), paste0("Showing Top ",min," + 'Other'"))

  list("phylo" = phylo, "plot" = plot, "pal" = tPalette)
}
